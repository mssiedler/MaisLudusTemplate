...
Template Unity com ênfase na criação de Apps voltado ao aprendizado de crianças através da mecânica *Drag and Drop*. O desenvolvimento segue preceitos de Tecnologia Assististiva e busca oferecer aplicações acessíveis, especialmente para crianças com Transtorno do Espectro Autista(TEA). 

O Template foi criando em uma iniciativa conjunta entre IFSUL/IFPEL, sendo criado a partir de iniciativas realizadas no projeto [+Ludus](https://linktr.ee/maisludus), Projeto de Inovação Tecnológica que visa a criação e disponibilização de aplicativos inclusivos, com ênfase em crianças com TEA.
...
O Template oferece a possibilidade de criação de Aplicativos sem a necessidade de codificação da lógica das fases. É possível, usando os prefabs disponibilizados e o *Inspector* da Unity, configurar as fases e utilizar os recursos desenvolvidos para apoiar a criação de Aplicativos. 
...
6) Criar uma pasta chamada **objetosons** e adicionar na pasta os arquivos de áudio relacionados a cada objeto que pode ser pareado.
...
A primeia observação importante é que, tanto o objeto quanto a sombra, apresentam as imagens na **mesma ordem** e com correspondencia de tamanho entre os objetos e as sombras para uma correta sobreposição.
...
* A fase deverá apresentar duas vezes um elemento para parear, duas vezes dois elementos para parear e, por fim, uma vez quatro elementos para parear.
...
3) Arraste a Camera principal ao Canvas do prefab conforme o exemplo a seguir.

![Ajuste da Câmera](midia/exemploF1.gif)
...
Deve ser definida a pasta na qual os assets estão inseridos (no nosso caso **teste**, a configuração dos níveis e, por fim, definir o nome da cena final, que foi especificada como **FinalCode**, uma cena que vem no pacote de exemplos do Template).

Essa configuração é feita no Script do Painel do Prefab e é feita ajustando os parâmetros conforme a especificação da Aplicação:
...
O Template oferece algumas customizações que podem auxiliar o desenvolvedor na criação dos aplicativos. Basicamente é possível customizar duas funcionalidades:

* Adicionar um conteúdo auxiliar a sombra do objeto deve ser pareado;  e
* Substituir a imagem do objeto quando ocorre o pareamento.
...
O que mudou foi a adição do arquivo **auxiliar.png** e da pasta **auxiliarsons**, com os áudios do conteúdo auxiliar.
...
![Cena sem o conteúdo auxiliar](midia/conteudosemauxiliar.png)

A fase **exemplo** consiste em um balde "pintar" a sombra. Nesse caso é fundamental ter o conteúdo auxiliar, pois sem ele não tem como saber a cor que deve pintar cada quadrado. 
Para habilitar o conteúdo auxiliar na Cena  é necessário habilitar a caixa de seleção no script de confguração da fase conforme a imagem a seguir.

![Cena sem o conteúdo auxiliar](midia/conteudoauxiliardetalhe.png)

Após marcar o conteúdo a apresentação do conteúdo fica da seguinte forma:

![Cena sem o conteúdo auxiliar](midia/conteudoauxiliaraposmarcar.png)
...
Primeiramente criamos um novo prefab, clonado a partir do  **sombraComAuxiliar**, que chamamos de **auxiliarBaixo**. Neste Prefab trocaremos a posição do conteúdo auxiliar para que este fique abaixo da sombra. Na figura a seguir é apresentado como ficou o novo preFab.

![ ](midia/auxiliarbaixo.png)
...
![ ](midia/substituiraoparear.png)

Como resultado final, quando ocorrer o pareamento a imagem do Balde será substituída pela do arquivo **objetoPareado.png**,  neste caso, a imagem é a mesma do conteúdo auxiliar, ficando conforme a imagem a seguir.
